 ...
 *
 * One compact 24px row of raw public market quotes (indices, FX, commodities,
 ...
 * height at 24px and publishes it as `--ticker-h` on <html> — AppShell subtracts
...
function Change({ pct }: { pct: number }) {
  ...
  return <span className="text-white/55">0.00%</span>;
}
...
function FlowChip({ label, cr }: { label: string; cr: number | null }) {
  ...
  const tone = cr > 0 ? 'text-emerald-dark' : cr < 0 ? 'text-red-dark' : 'text-white/55';
  return (
    <span className="flex items-center gap-1.5">
      <span className="text-white/75">{label}</span>
...
function StripContent({ data }: { data: TickerOut }) {
  return (
    <>
      ...
      <span className="flex items-center gap-1.5">
        <span className="text-white/75">NIFTY PCR</span>
...
export function TickerBar() {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty(TICKER_H_VAR, data ? '24px' : '0px');
    return () => {
