  ...
  return (
    <div className="shrink-0 overflow-hidden rounded-xl border border-line bg-surface shadow-sm" style={{ width, scrollSnapAlign: 'start' }}>
      <div className="flex items-center gap-2 border-b border-line px-3.5 py-2.5">
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg text-[13px]" style={{ background: `${rail.color}1A`, color: rail.color }} aria-hidden="true">{rail.icon}</span>
        <div className="min-w-0">
          <div className="truncate text-small font-semibold leading-tight text-ink">{rail.title}</div>
      ...
      {rail.rows.length === 0 ? (
        ...
        rail.rows.map((r, i) => {
          // Density pass (founder 2026-08-16): logo tile dropped in the narrow
          ...
          return r.href ? (
            <Link key={i} href={r.href} className="flex cursor-pointer items-center gap-2 border-b border-line px-3.5 py-2 last:border-b-0 hover:bg-surface-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-royal/40">
              {inner}
            </Link>
          ) : (
