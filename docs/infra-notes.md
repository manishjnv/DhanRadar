...
- **Reply/summary format.** Answer and summarize in simple-sentence pointers under standard sections — Implemented, Pending, Not implemented, Action for you, Dependencies, Issues, Deviations, Agent/model usage & % contribution, Improvement suggestions. No dense tables.
- **Phase-6 gate.** Do not start Phase 6 until the Resend dashboard shows `dhanradar.com` = Verified and a test send succeeds.
- The canonical, copy-ready **phase kickoff prompt** embedding these rules lives in `docs/DhanRadar_Implementation_Plan.md` → "Standard Phase Kickoff Prompt"; use it verbatim to start any phase.
- Treat all of the above as part of "done" in every phase's verification, not optional.
