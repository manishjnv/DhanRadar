...
### 2026-06-28 — Auto-deploy pipeline silently stalled 24h on the B80/B81 fail-closed (passwords never set)

- **Symptom:** the portfolio page never rendered live data in prod despite C1–C5 merging to `main` across 2026-06-27/28. A deploy pre-flight found the running `fastapi` image frozen at the #394 era (built 2026-06-27 12:37, `alembic` at `0051`, no `portfolio_read.py`/`serialization.py`, no `dhanradar_admin` role, RLS OFF) while the repo and the every-3-min auto-deploy poller had advanced to #408. Prod was up and serving — just stuck on day-old backend code.
- **Root cause:** B80/B81 added a fail-closed `Settings()` (`config.py`) that REQUIRES `DHANRADAR_APP_DB_PASSWORD` + `DHANRADAR_ADMIN_DB_PASSWORD` when `ENV=production`. Those two were never set in the prod-box `.env`. `deploy.sh` runs `alembic upgrade head` in a throwaway container whose `import dhanradar.config` constructs `Settings()` → `ValidationError` → `set -euo pipefail` aborts the deploy BEFORE the `compose up` recreate. The poller's last-deployed marker therefore stayed at the last good commit (`cf4585b` / #394) and every poll silently re-ran the same failing deploy (logged only to `/var/log/dhanradar-autodeploy.log`, no alert) for ~24h.
- **Fix:** paused the poller; took a fresh full backup (`20260628091632`, local + R2); generated + set both role passwords in `/opt/dhanradar/.env` (hex → URL/`.env`/SQL-safe; values never printed); ran `deploy.sh deploy` attended — build #408 → `alembic upgrade head` (`0052→0055`: grant-gap fix, RLS enable+FORCE+policy, B3 ledger cutover) → `sync_role_password` for both roles → recreate → smoke 200. Verified `db_role_hardened=true`, `alembic 0055 (head)`, RLS forced+policied on the 3 personal tables, roles correct (app NOBYPASSRLS / admin BYPASSRLS), zero permission-denied/500s, `/api/v1/mf/portfolio/latest` 401-gated. Re-enabled the poller with its marker pinned to HEAD (no-op).
- **Prevention:** (1) the two passwords now persist in the prod `.env` — the fail-closed will not recur unless they are cleared. (2) The real gap is NO ALERT on repeated deploy failures — a 24h silent stall is the actual bug → filed **B90** (alert when the poller fails N consecutive polls, via the existing Sentry/Telegram path). (3) Latent coupling: `deploy.sh`'s migrate step constructs the FULL `Settings()` even though `alembic` connects as the owner (`POSTGRES_PASSWORD`), so the app-role passwords gate a step that doesn't use them — noted in B90 as an optional hardening (minimal settings for migrate-only). Standing rule reinforced: a green poller log line ≠ a green deploy; verify the running image + `alembic current` against `origin/main`, not just `git log`.
- **Phase/area:** Infra / KVM4 deploy pipeline + B80/B81 prod activation.
