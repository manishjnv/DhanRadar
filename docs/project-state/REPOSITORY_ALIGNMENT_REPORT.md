...
- **Existing:** 8 internal containers, **no host port bindings**, cloudflared tunnel as sole ingress; local testing via `docker-compose.override.yml` (publishes ports, `COOKIE_SECURE=False`). Targets KVM4 shared-infra; reuses `etip_prometheus`/`etip_grafana`.
...
- **Existing/architecture:** reuse shared `etip_prometheus` + `etip_grafana` on KVM4; Sentry DSN.
