...
**Yahoo Finance — indices/macro** (src: query1.finance.yahoo.com, unofficial; sanctioned operationally 2026-06-22 per founder "public market data is DOM-allowed", added to this list 2026-06-27) — `GET https://query1.finance.yahoo.com/v8/finance/chart/{symbol}` (e.g. `^NSEI`, `^INDIAVIX`, `^GSPC`, `^BSESN`, `INR=X`, `BZ=F`); browser-like User-Agent required; response `chart.result[0].meta` (`regularMarketPrice`, `previousClose`) + `indicators`. **Primary macro/index source because NSE is geo-blocked on KVM4.** Unofficial → no SLA → Redis-cache 60s + 24h fallback. Also via the `yfinance` lib for NIFTY-50 advance/decline breadth (the mood worker reads the Redis cache **only** — never live-fetches, or it OOMs the 192 MB celery-mood container). Restriction: only raw public index **levels/% change** reach the DOM (the non-neg #2 carve-out — facts DhanRadar does not compute); never a computed score. `backend/dhanradar/market_data/providers/yahoo.py`.

**Upstox Analytics** (src: api.upstox.com/v2, sanctioned + added 2026-06-27) — read-only Analytics token (`UPSTOX_ANALYTICS_TOKEN`); supplies FII net flows, DII net flows, and the Nifty put-call ratio for the Mood Compass (twice daily, 09:00/16:00 IST). **Market-data ONLY — never order placement (non-neg #1).** DPDP-clean (market facts). PCR resolved from the live option chain (nearest-Thursday expiry). Graceful omission when the token is unset. `backend/dhanradar/market_data/providers/upstox.py`.

**GDELT DOC 2.0** (src: api.gdeltproject.org/api/v2/doc/doc, ToS confirmed 2026-06-10 / sanctioned B56-f5, added to this list 2026-06-27) — `GET .../doc?query=<india MF/market>&mode=ArtList&format=json`; **no API key**; returns India market/MF news headlines (title, source domain, url, seendate) for the news-sentiment Mood signal. Commercial use permitted **with attribution** (rendered in MarketNewsWidget). GDELT's own tone is discarded — DhanRadar derives sentiment via the governed OpenRouter gateway. 1 retry on 429 (2s backoff). `backend/dhanradar/news/`.
