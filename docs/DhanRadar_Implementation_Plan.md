...
**AMFI NAV** (src: portal.amfiindia.com, live-verified 2026-05-17) — `GET https://portal.amfiindia.com/spages/NAVAll.txt`, **semicolon-delimited**, header `Scheme Code;ISIN Div Payout/ISIN Growth;ISIN Div Reinvestment;Scheme Name;Net Asset Value;Date`, date fmt `DD-Mon-YYYY`, `-` for missing ISIN, category header lines interspersed (skip lines not matching 6-field split), updated ~21:00–22:00 IST business days (architecture doc's 23:30 IST NAV beat is safely after).
...
| **Public exposure / TLS** | **DEDICATED DhanRadar Cloudflare Tunnel** (NOT the existing one) — a *new named tunnel* created under the **`Manishjnvk@gmail.com`** CF account (the account that owns `dhanradar.com`), run as a **second cloudflared systemd instance** (`cloudflared-dhanradar.service`, own `/etc/cloudflared-dhanradar/config.yml` + own credentials JSON). Ingress: `dhanradar.com/api/*` → `http://dhanradar-fastapi:8000`, default → `http://dhanradar-nextjs:3000`, `http_status:404` catch-all. DNS = **CNAME** `dhanradar.com` → `<dhanradar-tunnel-id>.cfargotunnel.com`. | The existing tunnel (`6e263591-…`, locally-managed `/etc/cloudflared/config.yml`) carries **`ssh.intelwatch.in → ssh://localhost:22` — the box's SSH lifeline** — plus `ti.intelwatch.in`; a bad edit there could lock us out of the box. It is also account-scoped to the *intelwatch* CF account, so it **cannot** route `dhanradar.com` (different account) anyway. A separate tunnel = zero blast radius on intelwatch/SSH + correct account scoping. cloudflared is ~tiny RAM. Still no DhanRadar nginx/Caddy container — ingress path-routing replaces the architecture doc's "nginx routes /api". |
| **Observability** | **REUSE** `etip_prometheus` (add a DhanRadar scrape job to its config) + `etip_grafana` (add DhanRadar dashboards/alerts) | Shared Prom+Grafana already running; zero extra RAM |
| **PostgreSQL** | **OWN dedicated container** `timescale/timescaledb-ha:pg16` (bundles timescaledb + pgvector), mem-cap ~1.2 GB (headroom allows it) | **Mandatory, two reasons:** (1) shared `etip_postgres` is plain pg16 with **no TimescaleDB** — `mf_nav_history`/`mood_history` need it; (2) regulatory isolation — DhanRadar holds DPDP consent, 7-yr SEBI audit, financial holdings; independent `pg_dump`→R2, own blast-radius, CERT-In log isolation. Internal-only (Docker network, no host port). |
| **Redis** | **OWN dedicated container** `redis:7-alpine`, ~256 MB, `maxmemory-policy noeviction` | `etip_redis` is another product's + NOAUTH; budget-governor `ai:budget:*` keys must never be LRU-evicted; architecture namespacing assumes a dedicated instance. Internal-only. |
...
1. **External accounts/DNS:** Cloudflare zone `dhanradar.com` ✅ **DONE** (account `Manishjnvk@gmail.com`, NS delegated, Free plan, verified 2026-05-17). The `dhanradar.com` **CNAME → `<dhanradar-tunnel-id>.cfargotunnel.com`** is created in step 4 from the *dedicated* tunnel (not the existing intelwatch tunnel). **Buy the $10 OpenRouter credit** (Phase 0 §0.1#3 — key alone = 50 req/day). Create **Resend** acct + verify `dhanradar.com` DKIM (Phase 0 §0.1#1 — NOT SendGrid). Private repo under `github.com/manishjnv`. Cloudflare **R2** bucket + R2-scoped S3 token.
2. **Phase-1 investigation tasks (do first; record in `infra-notes.md`):** (a) **cloudflared = RESOLVED** (host `cloudflared.service`, locally-managed `/etc/cloudflared/config.yml`, tunnel `6e263591-…` carrying `ssh.intelwatch.in`+`ti.intelwatch.in`) → **do NOT touch it**; instead the prerequisite is `cloudflared tunnel login` authenticated to the **`Manishjnvk@gmail.com`** CF account (produces `~/.cloudflared/cert.pem` scoped to that account/zone) — needed to create the dedicated tunnel in step 4; (b) how `etip_prometheus` discovers scrape targets (`/opt/intelwatch/.../prometheus.yml`?) and how to add a DhanRadar job; (c) confirm `timescale/timescaledb-ha:pg16` ships `vector`; verify/install `pg_partman` + `pg_cron`; (d) pick a clearly-free host port **only if** one is ever needed (most DhanRadar traffic stays internal+tunnel) — avoid the bound-port list above.
...
4. **Dedicated DhanRadar tunnel — ✅ DONE 2026-05-17 (record of what exists + corrected commands; never edit `/etc/cloudflared/config.yml` or `cloudflared.service` = intelwatch's etip-ssh + the box SSH lifeline):**
   - **Tunnel:** `dhanradar` = `df2c5ae4-4d21-4052-83d4-12cbabbcd551`; isolated cert `/etc/cloudflared-dhanradar/cert.pem` (scoped to `Manishjnvk@gmail.com`, the account owning `dhanradar.com`), credentials `/etc/cloudflared-dhanradar/dhanradar.json`. `cloudflared tunnel --origincert /etc/cloudflared-dhanradar/cert.pem create --credentials-file /etc/cloudflared-dhanradar/dhanradar.json dhanradar`.
   - **DNS:** `dhanradar.com` proxied CNAME → `df2c5ae4-….cfargotunnel.com`, verified `HTTP/2 200` end-to-end. **GOTCHA (cost us a remediation): `cloudflared tunnel route dns <NAME> …` resolves the tunnel via the *default* `/etc/cloudflared/config.yml` (= etip-ssh `6e263591`) and mis-targets the CNAME.** Correct form is the **explicit UUID + overwrite**: `cloudflared tunnel --origincert /etc/cloudflared-dhanradar/cert.pem route dns --overwrite-dns df2c5ae4-4d21-4052-83d4-12cbabbcd551 dhanradar.com` — or set it in the CF dashboard DNS UI (Content must read `dhanradar`, not `etip-ssh`).
...
8. **Cost-governor** Redis keys `ai:budget:free:today`/`ai:budget:premium:today` with `EXPIREAT` next-UTC-midnight, `budget_guard()` stub; add DhanRadar alerts (mem>80%, p99>500ms, AI budget>900) to **reused** `etip_grafana`; Sentry DSN.
...
**Verification:** `docker compose up` → all 8 DhanRadar containers healthy; `https://dhanradar.com/api/v1/health` → 200 **through the Cloudflare Tunnel** with valid CF cert; **intelwatch's existing hostnames still resolve** after cloudflared reload (curl each); `psql` in own container `\dx` lists 5 extensions; Celery boots per queue; box `free -m` still ≥4 GB free under load; DhanRadar Redis `maxmemory-policy`=`noeviction`; a DhanRadar metric appears in `etip_prometheus`/`etip_grafana`. Grep guards: no `@app.on_event`, no missing `timezone`, no host `:80/:443` binding, no DhanRadar nginx/Caddy container.
**Anti-pattern guards:** Phase 0 §0.3 + (new) never edit cloudflared ingress or `etip_prometheus` config without a backup + post-reload verification that the intelwatch product still works; DhanRadar containers join only the `dhanradar` Docker network and never read/write `etip_postgres`/`etip_redis`; no host-port collision with the bound-port list.
