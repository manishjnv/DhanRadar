...
DhanRadar runs on the same KVM4 host as the existing `shared` stack. The host's cloudflared binary (version 2026.3.0) and the `shared-ssh` tunnel are **shared infrastructure** — do not touch them. DhanRadar uses its **own dedicated tunnel** provisioned separately.

### Dedicated tunnel

- Tunnel ID: `<DHANRADAR-TUNNEL-ID>`
...
The host's `/etc/cloudflared/config.yml` points at the `shared-ssh` tunnel. Running
...
cloudflared tunnel --config /etc/cloudflared-dhanradar/config.yml \
  route dns <DHANRADAR-TUNNEL-ID> dhanradar.com \
  --overwrite-dns
```
...
> **The following items belong to the shared stack and must not be touched:**
>
> - Tunnel `<SHARED-SSH-TUNNEL-ID>-...` (`shared-ssh`)
> - Host file `/etc/cloudflared/config.yml`
> - Systemd service `cloudflared.service`
> - Any container whose name starts with `shared_`
> - Host cloudflared binary at version `2026.3.0` — do not upgrade it

Modifying any of these will break the shared SSH reverse tunnel and cut off remote access to KVM4.
