...
async def dedup_lookup(redis: Any, source_hash: str) -> Optional[str]:
    """Return an existing job_id for this CAS hash, or None."""
    return await redis.get(_DEDUP_PREFIX + source_hash)


async def dedup_record(redis: Any, source_hash: str, job_id: str) -> None:
    await redis.set(_DEDUP_PREFIX + source_hash, job_id, ex=_DEDUP_TTL)
