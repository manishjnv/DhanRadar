...
from dhanradar.mf.manual_ingest import ALLOWED_EXTENSIONS, MAX_BYTES, intake_file
from dhanradar.models.mf import MfManualIngestFile

from .manual_ingest_schemas import ManualIngestFileRow, UploadBatchResponse, UploadFileResult
...
async def upload_disclosure_files(
    ...
    """Upload up to 10 disclosure files at once. Pre-flight validates EVERY file's
    extension + size before any is persisted — a single bad file 422/413s the
...
async def list_disclosure_files(
    ...
    rows = (
        await db.scalars(
            select(MfManualIngestFile)
            .order_by(MfManualIngestFile.received_at.desc())
            .limit(limit)
        )
    ).all()
    return [
