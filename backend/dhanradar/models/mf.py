...
class UserFundScore(Base):
    ...
    confidence_band: Mapped[str] = mapped_column(Text, nullable=False)
    verb_label: Mapped[str] = mapped_column(Text, nullable=False)
    # Engine diagnostic flags (G10/migration 0023): qualitative string tags only
    ...
    flags: Mapped[Optional[list]] = mapped_column(JSONB, nullable=True, server_default=text("'[]'"))
