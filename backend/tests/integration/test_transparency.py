...
async def test_transparency_flags_drive_drivers_and_what_would_change(
    ...
    assert "less often" in drivers_joined  # low_liquidity

    # what_would_change is present, non-empty, and includes the category-relative
    # methodology explainer + a liquidity-specific path (plain-language wording,
    # so we assert the user-facing phrasing, not the internal "liquidity" jargon).
    ...
    assert "traded more often" in wwc_joined  # low_liquidity path, plain-language
